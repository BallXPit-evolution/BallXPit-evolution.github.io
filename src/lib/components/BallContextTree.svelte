...
>
	<div
		class="relative flex h-35 shrink-0 items-center justify-center border-b border-indigo-900/50 bg-linear-to-b from-indigo-950/50 to-transparent px-4 py-2"
	>
		{#if selectedBall.img}
	...
	<div
		class="cursor-pointer px-4 py-2 font-serif text-lg leading-relaxed text-slate-300 italic hover:bg-white/5"
	...
	{#if selectedBall.parents && selectedBall.parents.length > 0}
		<div class="border-t border-indigo-900/30 bg-indigo-950/20 px-4 py-2">
			<h5 class="text-[9px] font-black tracking-[0.3em] text-indigo-500 uppercase sm:text-[10px]">
				Fusion Recipes
			</h5>
	...
	{#if childEvolutions.length > 0}
		<div class="border-t border-indigo-900/30 bg-indigo-950/10 px-4 py-2">
	...
	{#if (selectedBall.statusEffect && selectedBall.statusEffect.length > 0) || selectedBall.damageType.length > 0}
		<div class="flex shrink-0 flex-wrap border-t border-indigo-900/50 bg-[#050507]/80 px-4 py-2">
			...
			{#each selectedBall.damageType as type, index (index)}
				<span
					class="mr-2 rounded border border-indigo-500/20 bg-indigo-950/40 px-4 py-2 text-xs font-black tracking-[0.2em] text-indigo-300 uppercase"
				>
					{type}
				</span>
			{/each}
